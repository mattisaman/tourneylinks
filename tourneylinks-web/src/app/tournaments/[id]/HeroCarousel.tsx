...
export default function HeroCarousel({ tournament, heroImages, themeColor }: HeroCarouselProps) {
  ...
  return (
    <div style={{ position: 'relative', minHeight: '60vh', padding: '8rem 0 0 0', display: 'flex', alignItems: 'flex-end', paddingBottom: '6rem', overflow: 'hidden' }}>
      ...
      {/* Dynamic Gradient Array mimicking the Homepage Premium feel but with variable Hex Color */}
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundImage: 'linear-gradient(to right, rgba(7, 21, 16, 0.95) 0%, rgba(7, 21, 16, 0.75) 40%, rgba(7, 21, 16, 0.4) 100%), linear-gradient(to top, rgba(7, 21, 16, 1) 0%, transparent 40%)', zIndex: -3 }}></div>
      
      {/* Light Flare (Uses Theme Hex if Host provided one, otherwise standard Gold) */}
      ...
      <div className="section-wrapper" style={{ width: '100%', position: 'relative', zIndex: 1 }}>
        ...
        <div className="hero-sub" style={{ display: 'flex', gap: '1.25rem', flexWrap: 'wrap', alignItems: 'center', textShadow: '0 2px 15px rgba(0,0,0,0.8)', color: '#f8faf9', marginTop: '1.5rem' }}>
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', fontSize: '1.05rem', fontWeight: 500, background: 'rgba(255,255,255,0.06)', padding: '0.6rem 1.25rem', borderRadius: '50px', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ color: isCustomTheme ? `rgb(${themeRgb})` : 'var(--gold)', fontSize: '1.2rem' }}>📍</span> {tournament.courseName}
          </span>
          
          <span style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', fontSize: '1.05rem', fontWeight: 500, background: 'rgba(255,255,255,0.06)', padding: '0.6rem 1.25rem', borderRadius: '50px', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ color: isCustomTheme ? `rgb(${themeRgb})` : 'var(--gold)', fontSize: '1.2rem' }}>🗓️</span> {formatDate(tournament.dateStart)}
          </span>
          
          <span style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', fontSize: '1.05rem', fontWeight: 500, background: 'rgba(255,255,255,0.06)', padding: '0.6rem 1.25rem', borderRadius: '50px', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ color: isCustomTheme ? `rgb(${themeRgb})` : 'var(--gold)', fontSize: '1.2rem' }}>⛳</span> {tournament.format || '18 Hole Round'}
          </span>
