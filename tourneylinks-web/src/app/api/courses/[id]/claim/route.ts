...
export async function POST(req: Request, props: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { roleTitle, directPhone, pgaCardImageUrl, coAdminEmails } = await req.json();
      ...
      try {
         ...
         if (isMock) {
            ...
            extractedText = `[AI VISION EXTRACTION]\nName Match: 99% Confident\nFacility Association: 98% Confident\nCard Type: Official PGA Membership Card`;
             ...
             // In production we fetch the UploadThing buffer and pass it into the Vision model.
             ...
             const response = await ai.models.generateContent({
                ...
                contents: [
                   { role: 'user', parts: [ 
                       { text: "Extract the exact printed name and facility name from this PGA ID card. Summarize the role or text matching." },
    ...
    await db.insert(course_claims).values({
       ...
       coAdminEmails: coAdminEmails && coAdminEmails.length > 0 ? JSON.stringify(coAdminEmails) : null,
       status: 'PENDING'
    });
