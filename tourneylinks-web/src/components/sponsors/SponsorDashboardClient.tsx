  ...
  if (!sponsorData || sponsorData.length === 0) {
    return (
      <div className="min-h-[calc(100vh-80px)] bg-[#FAF9F6] p-16 text-center">
         ...
         <Link href="/tournaments" className="bg-[var(--gold)] text-black font-bold px-6 py-3 rounded-md shadow-lg">Discover Inventory</Link>
      </div>
    );
  }
    ...
    <div className="min-h-[calc(100vh-80px)] bg-[#FAF9F6] font-sans flex">
      ...
      <div className="w-[280px] bg-[#0a120e] text-white flex flex-col shrink-0 min-h-full">
        ...
        <nav className="flex-1 px-4 space-y-1">
          <button onClick={() => setActiveTab('analytics')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-md font-bold text-sm transition-all ${activeTab === 'analytics' ? 'bg-[rgba(212,175,55,0.15)] text-[var(--gold)]' : 'text-neutral-400 hover:text-white hover:bg-white/5'}`}>
            <LayoutDashboard className="w-5 h-5" /> Campaign Analytics
          </button>
          <button onClick={() => setActiveTab('profile')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-md font-bold text-sm transition-all ${activeTab === 'profile' ? 'bg-[rgba(212,175,55,0.15)] text-[var(--gold)]' : 'text-neutral-400 hover:text-white hover:bg-white/5'}`}>
            <Award className="w-5 h-5" /> Brand Profile
          </button>
          <button onClick={() => setActiveTab('media')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-md font-bold text-sm transition-all ${activeTab === 'media' ? 'bg-[rgba(212,175,55,0.15)] text-[var(--gold)]' : 'text-neutral-400 hover:text-white hover:bg-white/5'}`}>
            <UploadCloud className="w-5 h-5" /> Media Kit Studio
          </button>
          <button onClick={() => setActiveTab('tax')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-md font-bold text-sm transition-all ${activeTab === 'tax' ? 'bg-[rgba(212,175,55,0.15)] text-[var(--gold)]' : 'text-neutral-400 hover:text-white hover:bg-white/5'}`}>
        ...
        <div className="p-6 mt-autobg-black/20">
           <button 
             ...
             className="w-full flex items-center justify-center gap-2 bg-[var(--gold)] text-black font-extrabold py-3.5 rounded-md text-sm shadow-[0_0_20px_rgba(212,175,55,0.3)] hover:scale-105 transition-transform"
           >
      ...
      <div className="flex-1 p-10 lg:p-14 overflow-y-auto max-h-[calc(100vh-80px)]">
        ...
        <div className="flex justify-between items-center mb-10">
           ...
           {(activeTab === 'profile' || activeTab === 'media') && (
             <button 
               onClick={handleSave}
               className="flex items-center gap-2 bg-[#0a120e] text-[var(--gold)] px-6 py-2.5 rounded-md font-bold text-sm hover:bg-black transition-colors shadow-lg"
             >
        ...
        {activeTab === 'analytics' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                <div className="bg-white p-6 rounded-md border border-[#e8eada] shadow-sm">
             ...
             <div className="bg-white rounded-md border border-[#e8eada] shadow-sm overflow-hidden">
                ...
                <div className="p-0">
                   {sponsorData.map((d: any, i: number) => (
                     <div key={i} className="flex items-center justify-between p-6 border-b border-[#e8eada] last:border-b-0 hover:bg-neutral-50 transition-colors">
                        ...
                        <button className="text-sm font-bold text-[var(--gold)] border border-[#e8eada] px-4 py-2 rounded-md bg-white hover:bg-neutral-50">View Live Telemetry</button>
                     </div>
                   ))}
                </div>
             </div>
        ...
        {activeTab === 'profile' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl">
             <div className="bg-white rounded-md border border-[#e8eada] shadow-sm overflow-hidden mb-8">
               <div className="p-6 border-b border-[#e8eada] flex items-center gap-3">
                  <div className="w-10 h-10 bg-[rgba(212,175,55,0.1)] text-[var(--gold)] rounded-md flex items-center justify-center shrink-0">
               ...
               <div className="p-8 space-y-6">
                 <div className="grid grid-cols-2 gap-6">
                   <div>
                     ...
                     <input type="text" value={profile.companyName} onChange={e => setProfile({...profile, companyName: e.target.value})} className="w-full border border-[#e8eada] rounded-md px-4 py-3 font-medium text-sm focus:ring-2 focus:ring-[var(--gold)] outline-none" />
                   </div>
                   <div>
                     <label className="block text-xs font-bold text-neutral-400 uppercase tracking-widest mb-2">Industry Segment</label>
                     <input type="text" value={profile.industrySegment} onChange={e => setProfile({...profile, industrySegment: e.target.value})} className="w-full border border-[#e8eada] rounded-md px-4 py-3 font-medium text-sm focus:ring-2 focus:ring-[var(--gold)] outline-none" />
                   </div>
                 </div>
                 <div>
                     <label className="block text-xs font-bold text-neutral-400 uppercase tracking-widest mb-2">Activation Region</label>
                     <input type="text" value={profile.locationName} onChange={e => setProfile({...profile, locationName: e.target.value})} className="w-full border border-[#e8eada] rounded-md px-4 py-3 font-medium text-sm focus:ring-2 focus:ring-[var(--gold)] outline-none" />
                 </div>
                 <div>
                     ...
                     <textarea value={profile.overviewText} rows={4} onChange={e => setProfile({...profile, overviewText: e.target.value})} className="w-full border border-[#e8eada] rounded-md px-4 py-3 font-medium text-sm focus:ring-2 focus:ring-[var(--gold)] outline-none resize-y" placeholder="Describe your sponsorship objectives..." />
                 </div>
               </div>
             </div>
             ...
             <div className="bg-white rounded-md border border-[#e8eada] shadow-sm overflow-hidden">
               ...
               <div className="p-8 grid grid-cols-2 gap-6">
                 <div>
                     ...
                     <input type="text" value={profile.contactName} onChange={e => setProfile({...profile, contactName: e.target.value})} className="w-full border border-[#e8eada] rounded-md px-4 py-3 font-medium text-sm focus:ring-2 focus:ring-[var(--gold)] outline-none" />
                 </div>
                 <div>
                     <label className="block text-xs font-bold text-neutral-400 uppercase tracking-widest mb-2">Rep Title</label>
                     <input type="text" value="VP of B2B Marketing" disabled className="w-full border border-[#e8eada] rounded-md px-4 py-3 font-medium text-sm bg-neutral-50 text-neutral-400 outline-none" />
                 </div>
                 <div>
                     <label className="block text-xs font-bold text-neutral-400 uppercase tracking-widest mb-2">Direct Phone</label>
                     <input type="text" value={profile.contactPhone} onChange={e => setProfile({...profile, contactPhone: e.target.value})} className="w-full border border-[#e8eada] rounded-md px-4 py-3 font-medium text-sm focus:ring-2 focus:ring-[var(--gold)] outline-none" />
                 </div>
                 <div>
                     <label className="block text-xs font-bold text-neutral-400 uppercase tracking-widest mb-2">Direct Email</label>
                     <input type="text" value={profile.contactEmail} onChange={e => setProfile({...profile, contactEmail: e.target.value})} className="w-full border border-[#e8eada] rounded-md px-4 py-3 font-medium text-sm focus:ring-2 focus:ring-[var(--gold)] outline-none" />
                 </div>
               </div>
             </div>
          </div>
        ...
        {activeTab === 'media' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
               ...
               {/* Primary Logo */}
               <div className="bg-white p-8 rounded-md border border-[#e8eada] shadow-sm flex flex-col items-center text-center">
                  <div className="w-16 h-16 bg-neutral-100 text-neutral-400 rounded-md flex items-center justify-center mb-4">
                  ...
                  {uploadStates.logo === true ? (
                     <div className="w-full py-3 bg-[#2ecc71]/10 text-[#2ecc71] font-bold rounded-md text-sm flex items-center justify-center gap-2">
                        <CheckCircle2 className="w-4 h-4" /> Auto-Deployed
                     </div>
                  ) : uploadStates.logo === 'uploading' ? (
                     <div className="w-full py-3 bg-neutral-100 text-neutral-500 font-bold rounded-md text-sm animate-pulse">Uploading...</div>
                  ) : (
                     <button onClick={() => simulateUpload('logo')} className="w-full border-2 border-dashed border-[#e8eada] hover:border-[var(--gold)] hover:bg-[rgba(212,175,55,0.05)] text-[var(--gold)] font-bold py-8 rounded-md transition-all flex flex-col items-center gap-2">
               ...
               {/* Pitch Deck */}
               <div className="bg-white p-8 rounded-md border border-[#e8eada] shadow-sm flex flex-col items-center text-center">
                  <div className="w-16 h-16 bg-rose-50 text-rose-500 rounded-md flex items-center justify-center mb-4">
                  ...
                  {uploadStates.pdff === true ? (
                     ...
                     <button onClick={() => simulateUpload('pdff')} className="w-full border-2 border-dashed border-[#e8eada] hover:border-rose-300 hover:bg-rose-50 text-rose-500 font-bold py-8 rounded-md transition-all flex flex-col items-center gap-2">
                        <UploadCloud className="w-6 h-6" />
               ...
               {/* Activation Video */}
               <div className="bg-white p-8 rounded-md border border-[#e8eada] shadow-sm flex flex-col items-center text-center">
                  <div className="w-16 h-16 bg-blue-50 text-blue-500 rounded-md flex items-center justify-center mb-4">
                  ...
                  {uploadStates.mp4 === true ? (
                     ...
                     <button onClick={() => simulateUpload('mp4')} className="w-full border-2 border-dashed border-[#e8eada] hover:border-blue-300 hover:bg-blue-50 text-blue-500 font-bold py-8 rounded-md transition-all flex flex-col items-center gap-2">
                        <UploadCloud className="w-6 h-6" />
