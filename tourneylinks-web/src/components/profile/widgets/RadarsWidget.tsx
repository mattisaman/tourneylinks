...
export default function RadarsWidget({ userRadars }: { userRadars: any[] }) {
  return (
    <WidgetCard className="h-full">
       <h3 className="hero-eyebrow !mb-4 flex items-center gap-3 !bg-none !border-none !shadow-none !backdrop-filter-none !p-0 !text-sm" style={{ borderBottom: '1px solid rgba(245,240,232,0.1)', paddingBottom: '1rem' }}>
         <MapPin size={20} style={{ color: 'var(--gold)' }} /> Course Radars
       </h3>
       {userRadars.length === 0 ? (
             ...
             <a href={`/courses/${row.course.id}`} key={row.radar.id} className="group block py-4 hover:bg-white/[0.03] px-3 rounded-xl transition-colors" style={{ borderBottom: '1px solid rgba(245,240,232,0.05)' }}>
               <div className="flex items-center justify-between">
                 <div className="min-w-0 pr-4">
                   <div className="text-xl font-semibold truncate group-hover:text-[var(--gold)] transition-colors" style={{ color: 'var(--cream)', fontFamily: 'var(--font-serif), serif' }}>{row.course.name}</div>
                   <div className="text-xs mt-2 uppercase tracking-wider font-medium" style={{ color: 'rgba(245,240,232,0.6)' }}>{row.course.city}, {row.course.state}</div>
                 </div>
